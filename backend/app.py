from flask import Flask, request, jsonify
from flask_cors import CORS
import pickle, random, re, nltk
from nltk.stem import WordNetLemmatizer
from nltk.tokenize import word_tokenize

nltk.download('punkt',     quiet=True)
nltk.download('punkt_tab', quiet=True)
nltk.download('wordnet',   quiet=True)

app = Flask(__name__)
CORS(app)

# ════════════════════════════════════════
#  MODEL LOADING
# ════════════════════════════════════════

# ── Logistic Regression (required)
try:
    lr_model      = pickle.load(open('../models/lr_model.pkl',      'rb'))
    vectorizer    = pickle.load(open('../models/vectorizer.pkl',     'rb'))
    label_encoder = pickle.load(open('../models/label_encoder.pkl', 'rb'))
    print("✅ LR model loaded!")
except Exception as e:
    raise RuntimeError(f"❌ Failed to load LR model (required): {e}")

# ── LSTM (optional)
try:
    from tensorflow.keras.models import load_model
    from tensorflow.keras.preprocessing.sequence import pad_sequences
    lstm_model     = load_model('../models/lstm_model.keras')                      # ← .keras format
    tokenizer_lstm = pickle.load(open('../models/tokenizer_lstm.pkl', 'rb'))
    LSTM_AVAILABLE = True
    print("✅ LSTM model loaded!")
except Exception as e:
    lstm_model     = None
    tokenizer_lstm = None
    LSTM_AVAILABLE = False
    print(f"⚠️  LSTM skipped: {e}")

# ── BERT (optional)
try:
    from transformers import BertTokenizer, BertForSequenceClassification
    import torch
    bert_model     = BertForSequenceClassification.from_pretrained('../models/bert_emotion_model')
    bert_tokenizer = BertTokenizer.from_pretrained('../models/bert_emotion_model')
    bert_model.eval()
    BERT_AVAILABLE = True
    print("✅ BERT model loaded!")
except Exception as e:
    bert_model     = None
    bert_tokenizer = None
    BERT_AVAILABLE = False
    print(f"⚠️  BERT skipped: {e}")

# ════════════════════════════════════════
#  TEXT PREPROCESSING
# ════════════════════════════════════════

lemmatizer = WordNetLemmatizer()

def clean_text(text: str) -> str:
    text = text.lower()
    text = re.sub(r'\s+', ' ', text).strip()
    text = re.sub(r'[^\w\s.!?]', '', text)
    return text

def tokenize_lemmatize(text: str) -> str:
    tokens = word_tokenize(text)
    return " ".join([lemmatizer.lemmatize(w) for w in tokens])

def preprocess(text: str) -> str:
    return tokenize_lemmatize(clean_text(text))

# ════════════════════════════════════════
#  EMOTION PREDICTION
# ════════════════════════════════════════

def predict_emotion(text: str, model_choice: str = 'lr') -> str:
    cleaned = preprocess(text)

    if model_choice == 'lstm' and LSTM_AVAILABLE:
        seq  = tokenizer_lstm.texts_to_sequences([cleaned])
        pad  = pad_sequences(seq, maxlen=150, padding='post', truncating='post')
        pred = lstm_model.predict(pad, verbose=0)
        return label_encoder.inverse_transform([pred.argmax()])[0]

    if model_choice == 'bert' and BERT_AVAILABLE:
        import torch
        inputs = bert_tokenizer(
            text, return_tensors='pt',
            truncation=True, padding=True, max_length=128
        )
        with torch.no_grad():
            outputs = bert_model(**inputs)
        pred = outputs.logits.argmax(dim=1).item()
        return label_encoder.inverse_transform([pred])[0]

    # Default → Logistic Regression
    vec  = vectorizer.transform([cleaned])
    pred = lr_model.predict(vec)
    return label_encoder.inverse_transform(pred)[0]

def resolve_model_used(model_choice: str) -> str:
    """Returns which model was actually used (falls back to lr if unavailable)."""
    if model_choice == 'lstm' and LSTM_AVAILABLE:
        return 'lstm'
    if model_choice == 'bert' and BERT_AVAILABLE:
        return 'bert'
    return 'lr'

# ════════════════════════════════════════
#  RESPONSE BANK
# ════════════════════════════════════════

RESPONSES = {
    'sadness': {
        'casual':  [
            "I hear you. Want to tell me more?",
            "That sounds really heavy. I'm here to listen.",
        ],
        'comfort': [
            "You don't have to go through this alone.",
            "I care about how you're feeling right now.",
        ],
        'advice':  [
            "Try reaching out to someone you trust.",
            "Journaling or a short walk can sometimes help process heavy feelings.",
        ],
    },
    'anger': {
        'casual':  [
            "Sounds like something really got to you. What happened?",
            "I can feel the frustration. Want to talk about it?",
        ],
        'comfort': [
            "Your feelings are completely valid. I'm here.",
            "Let it out — this is a safe space.",
        ],
        'advice':  [
            "Try slow deep breaths before responding to the situation.",
            "Stepping away briefly can help you feel calmer.",
        ],
    },
    'fear': {
        'casual':  [
            "That sounds really unsettling. Do you want to talk through it?",
            "Fear can be overwhelming. I'm listening.",
        ],
        'comfort': [
            "You're not alone in this. I'm right here with you.",
            "Whatever you're scared of, you don't have to face it alone.",
        ],
        'advice':  [
            "Try naming 5 things you can see around you — it helps ground you.",
            "Talking to someone you trust about fears can really lighten the load.",
        ],
    },
    'joy': {
        'casual':  [
            "That's wonderful! Tell me more about what's making you happy!",
            "I love hearing that! What's going on?",
        ],
        'comfort': [
            "You deserve this happiness!",
            "I'm really glad you're in a good place right now.",
        ],
        'advice':  [
            "Write down what's making you happy — it helps on harder days.",
            "Sharing joy with others can multiply it!",
        ],
    },
    'neutral': {
        'casual':  [
            "I'm here if you want to share anything more.",
            "Okay! Anything on your mind you'd like to talk about?",
        ],
        'comfort': [
            "I'm here for you, whatever you need.",
            "You can always talk to me — no pressure at all.",
        ],
        'advice':  [
            "Checking in with yourself daily is a healthy habit.",
            "Even on quiet days, it helps to be kind to yourself.",
        ],
    },
}

# Emotion aliases
RESPONSES['surprise'] = RESPONSES['neutral']
RESPONSES['disgust']  = RESPONSES['anger']

# ════════════════════════════════════════
#  DISTRESS TRACKER
# ════════════════════════════════════════

distress_counter = {'count': 0}
DISTRESS_EMOTIONS = {'sadness', 'fear', 'anger'}
DISTRESS_THRESHOLD = 3

# ════════════════════════════════════════
#  ROUTES
# ════════════════════════════════════════

@app.route('/chat', methods=['POST'])
def chat():
    data         = request.json or {}
    user_input   = data.get('message', '').strip()
    mode         = data.get('mode', 'casual')
    model_choice = data.get('model', 'lr')

    if not user_input:
        return jsonify({'error': 'No message provided.'}), 400

    emotion = predict_emotion(user_input, model_choice)

    # Update distress counter
    if emotion in DISTRESS_EMOTIONS:
        distress_counter['count'] += 1
    else:
        distress_counter['count'] = 0

    # Auto-switch to comfort mode after threshold
    suggestion = None
    if distress_counter['count'] >= DISTRESS_THRESHOLD and mode == 'casual':
        mode       = 'comfort'
        suggestion = (
            "I've noticed you've been going through a lot. "
            "I've switched to comfort mode — you can also try the Advice button if you'd like."
        )

    pool     = RESPONSES.get(emotion, RESPONSES['neutral'])
    response = random.choice(pool.get(mode, pool['casual']))

    return jsonify({
        'response':   response,
        'emotion':    emotion,
        'model_used': resolve_model_used(model_choice),
        'suggestion': suggestion,
    })


@app.route('/status', methods=['GET'])
def status():
    """Health-check endpoint — shows which models are loaded."""
    return jsonify({
        'lr':   True,
        'lstm': LSTM_AVAILABLE,
        'bert': BERT_AVAILABLE,
    })


if __name__ == '__main__':
    app.run(debug=True)